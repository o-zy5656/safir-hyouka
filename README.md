...
- `docs/GUIDE_PUBLISH_BEGINNER.md` — **初心者向け・公開までの完全ガイド（まずここ）**
- `docs/PRE_DEPLOY_VERCEL.md` — 公開直前チェックリスト
...
cd backend
python -m pytest
python -m scripts.run_uat   # API が起動していること
```

既存 DB を Alembic 管理下に置く場合（初回のみ）:
...
以降のスキーマ変更は `alembic revision --autogenerate -m "説明"` → `alembic upgrade head`。
起動時の `migrate_schema()` は後方互換のため引き続き実行されます。
