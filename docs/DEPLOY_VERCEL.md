...
**公開デモ（`DEMO_MODE=true`）では ID・パスワード不要**で自動ログインします。  
画面上部のデモバナーが表示されていれば OK です。

ローカル開発やデモ API を手動確認する場合のみ、以下のアカウントが使えます。

| ロール | 社員ID | パスワード |
|--------|--------|------------|
| 管理者（ゲスト） | ADMIN001 | admin123 |
| 施設長（いなは） | DIR001 | pass123 |
| 施設長（そはら） | DIR002 | pass123 |
| 職員（いなは） | E101〜E108 | pass123 |
| 評価者1 | E010 | pass123 |
| 本部 | hq001 | pass123 |
